...
private func localOrRemoteBinaryTarget(name: String, url: String, checksum: String) -> Target {
    let relativePath = "LocalArtifacts/\(name)/\(name).xcframework"
    let absolutePath = URL(fileURLWithPath: #filePath)
